...
final class Menu_Icons_Front_End {
	...
	public static function get_svg_icon( $meta ) {
        ...
        $anthonyiconname = str_replace(".svg","",strtolower(basename($meta['url'])));
        //$anthonyicon = '<span class="icon-basketball navigation"></span>' . $anthonyiconname;
        $anthonyicon = '<span class="' . $anthonyiconname . ' navigation-icon" style="display:block"></span>';

        return $anthonyicon;

        /*
		return $anthonyicon . sprintf(
    ...
    public static function OLD_ANTHONY_get_svg_icon( $meta ) {
        $classes = sprintf( '%s _svg', self::get_icon_classes( $meta ) );
        $style   = self::get_icon_style( $meta, array( 'svg_width', 'vertical_align' ) );
